...
func TestAdapter_Name(t *testing.T) {
	a := &Adapter{}
	if a.Name() != "kilocode" {
		t.Errorf("Name() = %q, want %q", a.Name(), "kilocode")
	}
}
...
func TestAdapter_Detect(t *testing.T) {
	...
	t.Run("exists but is file not dir", func(t *testing.T) {
		home := t.TempDir()
		configPath := filepath.Join(home, ".kilocode")
...
func TestAdapter_ListItems(t *testing.T) {
	...
	setupHome := func(t *testing.T) string {
		...
		if err := os.WriteFile(filepath.Join(configDir, "settings.json"), []byte(`{"agent":"kilocode"}`), 0644); err != nil {
			t.Fatal(err)
		}
		// Rules directory
		...
		if err := os.WriteFile(filepath.Join(rulesDir, "coding.md"), []byte("# Rules"), 0644); err != nil {
			t.Fatal(err)
		}
		return home
	}
...
func TestAdapter_Backup(t *testing.T) {
	...
	items := []adapters.Item{
		{Category: "config", SourcePath: "~/.kilocode/settings.json", RelPath: "settings.json", IsDir: false, Hash: "sha256:abc", Size: 21},
	}
	backupDir := filepath.Join(t.TempDir(), "backup")
	if err := a.Backup(home, backupDir, items); err != nil {
		t.Fatalf("Backup: %v", err)
	}
	dstFile := filepath.Join(backupDir, "kilocode", "settings.json")
	data, err := os.ReadFile(dstFile)
	if err != nil {
		t.Fatalf("read backup file: %v", err)
	}
	if string(data) != `{"agent":"kilocode"}` {
		t.Errorf("backup content = %q, want %q", string(data), `{"agent":"kilocode"}`)
	}
}
...
func TestAdapter_Restore(t *testing.T) {
	a := &Adapter{}
	backupDir := filepath.Join(t.TempDir(), "backup")
	backupFile := filepath.Join(backupDir, "kilocode", "settings.json")
	...
	items := []adapters.Item{
		{Category: "config", SourcePath: "~/.kilocode/settings.json", RelPath: "settings.json", IsDir: false, Hash: "sha256:xyz", Size: 18},
	}
	home := t.TempDir()
	if err := a.Restore(backupDir, home, items); err != nil {
		t.Fatalf("Restore: %v", err)
	}
	restoredFile := filepath.Join(home, ".kilocode", "settings.json")
...
func TestAdapter_Backup_DirectoryItems(t *testing.T) {
	...
	items := []adapters.Item{
		{Category: "skills", SourcePath: "~/.kilocode/myskills", RelPath: "myskills", IsDir: true},
		{Category: "skills", SourcePath: "~/.kilocode/myskills/code-review.md", RelPath: "myskills/code-review.md", IsDir: false, Hash: "sha256:abc", Size: 13},
	}
	...
	dirDst := filepath.Join(backupDir, "kilocode", "myskills")
	...
	fileDst := filepath.Join(backupDir, "kilocode", "myskills", "code-review.md")
...
func TestAdapter_Backup_CopyError(t *testing.T) {
	...
	items := []adapters.Item{
		{Category: "config", SourcePath: "~/.kilocode/unreadable.txt", RelPath: "unreadable.txt", IsDir: false, Hash: "sha256:abc", Size: 4},
	}
...
func TestAdapter_Restore_CopyError(t *testing.T) {
	...
	backupDir := filepath.Join(t.TempDir(), "backup")
	srcFile := filepath.Join(backupDir, "kilocode", "secret.json")
	...
	items := []adapters.Item{
		{Category: "config", SourcePath: "~/.kilocode/secret.json", RelPath: "secret.json", IsDir: false, Hash: "sha256:xyz", Size: 16},
	}
